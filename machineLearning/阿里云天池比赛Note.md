...
- 归一化(`Nomalizer`)： 最大最小值归一化、零均值归一化
  ...
  - 通过梯度下降求解的模型，一般都需要归一化处理，否则收敛的时间会很长
- 定量特征二值化(`Binarizer`)
...
| 过拟合 | 高方差；算法训练的模型过多地表达了数据关系，此时很有肯能过多表达噪音数据 | 简化模型；收集更多数据；减少训练数据的噪音（修复数据错误，消除异常值）；减少特征数量；正则化（通过约束模型使其更简单，并降低过拟合的风险） |
