...
class BidirectionalAStar:
    def __init__(self, s_start, s_goal, heuristic_type):
        ...
        self.queue_fore = []  # OPEN set for forward searching
        self.queue_back = []  # OPEN set for backward searching
        self.visited_fore = []  # CLOSED set for forward
        self.visited_back = []  # CLOSED set for backward
    ...
    def init(self):
        ...
        heapq.heappush(self.queue_fore,
                       (self.f_value_fore(self.s_start), self.s_start))
        heapq.heappush(self.queue_back,
    ...
    def searching(self):
        ...
        while self.queue_fore and self.queue_back:
            # solve foreward-search
            _, s_fore = heapq.heappop(self.queue_fore)

            if s_fore in self.parent_back:
                s_meet = s_fore
                break

            self.visited_fore.append(s_fore)
            ...
            _, s_back = heapq.heappop(self.queue_back)

            if s_back in self.parent_fore:
                s_meet = s_back
                break

            self.visited_back.append(s_back)
        ...
        return self.extract_path(s_meet), self.visited_fore, self.visited_back
